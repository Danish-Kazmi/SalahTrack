...
export function getMonthRange(date) {
  ...
  return {
    startDateKey: formatDateKey(new Date(year, month, 1)),
    endDateKey: formatDateKey(new Date(year, month, lastDay)),
  };
}
...
export function calculateStats(data, visibleMonth) {
  ...
  return {
    ...
    qaza,
    monthProgress: Math.round((done / (daysInMonth * PRAYERS.length)) * 100),
  };
}
