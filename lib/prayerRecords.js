...
export async function fetchPrayerDataRange(userId, startDateKey, endDateKey) {
  const { data, error } = await supabase
    .from(TABLE_NAME)
    .select('date_key, prayers')
    .eq('user_id', userId)
...
export async function hasAnyPrayerRecords(userId) {
  const { data, error } = await supabase
    .from(TABLE_NAME)
    .select('date_key')
    .eq('user_id', userId)
...
export async function replacePrayerData(userId, prayerData) {
  ...
  const [{ error: upsertError }, { error: deleteStaleError }] = await Promise.all([
    supabase
      .from(TABLE_NAME)
    ...
    supabase
      ...
      .delete()
      .eq('user_id', userId)
      .not('date_key', 'in', `(${incomingDateKeys.join(',')})`),
  ]);
